...
for fold=1:5
    ...
    for d=1:size(numdepths,2)
    for f=1:size(numfeatures,2)
    ...
    classifier.setMaxDepth(numdepths(d));
    classifier.setNumFeatures(numfeatures(f));
...
%     Estlabeltrain(fold,:)= estimatedTrain';
    ...
    testLabels = test(:,end);
    Accuracy_test(fold,n,d,f) = (sum(estimatedTestLabels == testLabels) / length(testLabels)) * 100;
    
    % Compute accuracy of each fold train
%     trainlabel = train(:,end);
%     Accuracy_train(fold,n,d,f) = (sum(estimatedTrain == trainlabel) / length(trainlabel)) * 100;
